...
typedef Locations =
{
	miniSTAGEx:Float,
	miniSTAGEy:Float,
	miniSTAGEscalex:Float,
	miniSTAGEscaley:Float,
	miniOPPOx:Float,
	miniOPPOy:Float,
	miniOPPOscalex:Float,
	miniOPPOscaley:Float,
	miniGFx:Float,
	miniGFy:Float,
	miniGFscalex:Float,
	miniGFscaley:Float,
...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Philly Glow',
			"ported to other weeks by CS2020\nValue 1: 0/1/2 = OFF/ON/Reset Gradient"
		],
		[
			'Kill Henchmen',
	...
	**/
	...
	// ADD THESE WHEN INPUT CAN BE DETECTED
	var miniBF:FlxSprite;
	var miniGF:FlxSprite;
	// var miniOPPO:FlxSprite;
	...
	override function create()
	{
		...
		// IGNORE THIS!!!
		LOcations = tjson.TJSON.parse(Paths.getTextFromFile('data/DEBUG/CHART.json'));

		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('DEBUG/Chart/background/chartbg'));
		bg.antialiasing = ClientPrefs.data.antialiasing;
		bg.scrollFactor.set();
		// bg.color = 0xFF222222; //not used for current bg
		add(bg);
		...
		miniBF.animation.addByPrefix('idle', 'idle', 24, false);
		miniBF.animation.addByPrefix('hey', 'hey', 24, false);
		miniBF.animation.addByPrefix('sing[0]', 'sing1', 24, false);
		miniBF.animation.addByPrefix('sing[1]', 'sing2', 24, false);
		miniBF.animation.addByPrefix('sing[2]', 'sing3', 24, false);
		miniBF.animation.addByPrefix('sing[3]', 'sing4', 24, false);
		miniBF.animation.addByPrefix('none', 'none', 24, false);
		miniBF.scrollFactor.set();
		miniBF.scale.x = LOcations.miniBFscalex;
		miniBF.scale.y = LOcations.miniBFscaley;
		// GIRLFRIEND
		miniGF = new FlxSprite(LOcations.miniGFx, LOcations.miniGFy);
		miniGF.frames = Paths.getSparrowAtlas('DEBUG/Chart/minichars/miniGF');
		miniGF.animation.addByPrefix('DanceLeft', 'leftdance', 24, false);
		miniGF.animation.addByPrefix('DanceRight', 'rightdance', 24, false);
		miniGF.scrollFactor.set();
		miniGF.scale.x = LOcations.miniGFscalex;
		miniGF.scale.y = LOcations.miniGFscaley;
		// OPPONENT
		// miniOPPO = new FlxSprite(LOcations.miniOPPOx, LOcations.miniOPPOy);
		// miniOPPO.scale.x = LOcations.miniOPPOscalex;
		// miniOPPO.scale.y = LOcations.miniOPPOscaley;
		// STAGE
		var miniSTAGE:FlxSprite = new FlxSprite(LOcations.miniSTAGEx, LOcations.miniSTAGEy).loadGraphic(Paths.image('DEBUG/Chart/minichars/miniSTAGE'));
	...
	override function update(elapsed:Float)
	{
		...
		bpmTxt.text = Std.string(FlxMath.roundDecimal(Conductor.songPosition / 1000, 2))
			...
			+ curSec
			+ "\n\nBeat: "
			+ Std.string(curDecBeat).substring(0, 90)
			+ "\n\nStep: "
			+ curStep
			+ "\n\nBeat Snap: "
			+ quantization
		...
		if (metronome.checked && lastConductorPos != Conductor.songPosition)
		{
			...
			if (metroStep != lastMetroStep)
			{
				...
				if (miniGF != null)
				{
					DanceLeft = !DanceLeft;
					if (DanceLeft)
						miniGF.animation.play('DanceLeft');
					else
						miniGF.animation.play('DanceRight');
				}
				if (isSinging == false)
				{
					if (miniBF != null)
					{
						bop = !bop;
						if (bop)
							miniBF.animation.play('idle', true, false, 0);
						else
							miniBF.animation.play('none', true, false, 0);
					}
				}
				/////////////////////////////////////////////////////////////////
				// trace('Ticked');
