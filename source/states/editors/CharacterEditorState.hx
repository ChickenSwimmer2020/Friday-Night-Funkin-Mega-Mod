...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var dad:FlxSprite = new FlxSprite(dadPosition.x, dadPosition.y).loadGraphic(Paths.image('DEBUG/Char/silhouetteDad'));
		dad.antialiasing = ClientPrefs.data.antialiasing;
		dad.active = false;
		dad.offset.set(-4, 1);
		silhouettes.add(dad);

		var girlfriend:FlxSprite = new FlxSprite(gfPosition.x + 100, gfPosition.y + 105).loadGraphic(Paths.image('DEBUG/Char/silhouetteGF'));
		girlfriend.antialiasing = ClientPrefs.data.antialiasing;
		girlfriend.active = false;
		girlfriend.offset.set(0, 0);
		silhouettes.add(girlfriend);

		var boyfriend:FlxSprite = new FlxSprite(bfPosition.x, bfPosition.y + 350).loadGraphic(Paths.image('DEBUG/Char/silhouetteBF'));
